...
export async function GET(
  req: Request,
  { params }: { params: Promise<{ api_set_id: string }> }
) {
  ...
  try {
    ...
    if (!setData?.cards) throw new Error('No hay cartas');

    // Disparamos todas las requests en paralelo
    const cardDetails = await Promise.all(
      setData.cards.map((cardBrief: { id: string }) =>
        tcgdex.fetch('cards', cardBrief.id)
      )
    );

    // Construimos el array final con variantes dinámicas
    const fullCardsData = cardDetails
      .filter((detail): detail is NonNullable<typeof detail> => Boolean(detail))
      .flatMap(detail => {
        const variantsFound = Object.entries(detail.variants || {}) 
        .filter(([, value]) => Boolean(value)) 
        .map(([key]) => key); 

        if (variantsFound.length === 0) variantsFound.push('normal');

        return variantsFound.map(v => ({
    ...
    return NextResponse.json({
      ...
      message: `Se procesaron ${setData.cards.length} cartas base resultando en ${inserted?.length} variantes.`
    });
  } catch (error: unknown) {
    console.error(
      'Error en Deep Fetch:',
